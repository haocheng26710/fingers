...
DEV-05.03 independently consumes the same replay-validated hierarchy for development synthetic execution. It adds deterministic execution/session/reassembly/run identities but never changes the compiled plan, condition order, full NodeState maps or safety status. The execution ledger and synthetic session root are separate from both plan and rehearsal roots; DEV-05.02 completion grants no execution, operator or hardware authority. See `protocol-synthetic-execution.md`.
